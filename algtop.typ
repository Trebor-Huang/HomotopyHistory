...
在 1930 年代左右, 不少成果已经暗示了上同调的存在. 数学家 de Rham 将同调与微分形式联系在一起, 证明了 de Rham 定理. 这个定理最自然的表述是利用上同调, 即空间的 de Rham 上同调与其他的上同调同构. 1935 年, Kolmogorov 与 Alexander 正式定义了上同调. 不久后, Whitney 与 Čech 发现了上同调上的杯积与帽积. 这使得他们认识到 Poincaré 对偶可以利用帽积给出更好的表述. (介绍 cup cap)
