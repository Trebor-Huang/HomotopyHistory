...
上同调的万有系数定理在 1942 年由 Eilenberg 与 Mac Lane 随着 $"Hom"$ 与 $"Ext"$ 这两个代数概念一起提出. 其本质是描述函子 $"Hom"_ZZ (-, G)$ 对链复形的同调群的影响.
...
群的低维同调与上同调是早已被研究的课题, 如叉同态 (crossed homomorphism) 的概念对应 $H^1 (G; A)$.
$H^2 (G; A)$ 给出了以 $A$ 为正规子群, $G$ 为商群的群扩张的完全分类.

1941 年 Hopf 注意到 Hurewicz 映射 $pi_2 (X) -> H_2 (X; ZZ)$ 的余核由 $pi_1 (X)$ 完全确定, 并且给出了具体的计算公式. 这启发了 Eilenberg 与 Mac Lane 利用 $K(G, 1)$ 空间定义了群的任意维数同调与上同调. 他们证明了 $H^2 (G; A)$ 分类了群扩张, 将工作与传统的代数研究联系起来. 这将同调的语言正式引入了代数学.

群上同调的框架应用到类域论, 形成了 Galois 上同调的研究. 对交换代数与 Lie 代数的扩张问题, 也可以用上同调进行研究.

== 谱序列与层论

起初, 群上同调的计算仅限于那些 $K(G, 1)$ 是熟悉的拓扑空间的群 $G$. 在 1946 年, Lyndon 发现了一种计算方法, 取定一个正规子群 $N$, 从 $H^bullet (N)$ 与 $H^bullet (G \/ N; A)$ 出发计算 $H^bullet (G; A)$. 这种方法最后会产生上同调群的一系列 “零件”, 如果能知道这些零件如何组装 (例如可能的组装方式只有一种时), 就可以确定上同调.

数学家 Jean Leray 在二战期间被俘. 期间, 他发展了层论与层上同调. 层描述了能够局部定义并拼接的对象, 如连续函数、光滑函数等. 层上同调刻画了局部对象拼接成全局对象的阻碍, 例如局部有解的微分方程能否得到全局的解. 为了计算层上同调, 他还发展出了谱序列的理论. 1947 年, 根据 Cartan 的建议, Koszul 以滤链复形为核心概念, 发展出了谱序列的代数框架. Serre 注意到 Lyndon 早先的工作正是谱序列的雏形. 在战后, Cartan, Eilenberg, Serre 等人将谱序列与层的理论进一步发展成熟.

给定拓扑空间 $X$, 其上的*层*为每个开集 $U$ 赋予一个集合 $cal(F)(U)$, 表示这个开集上局部定义的数学对象. 例如连续函数层 $C^0$ 满足 $C^0(U)$ 是 $U$ 上连续函数的集合. 如果有两个开集 $U subset.eq V$, 那么有对应的限制映射 $cal(F)(V) -> cal(F)(U)$. 最后, 如果 $U = union.big_alpha U_alpha$ 被一族开集覆盖, 并且在每个开集上取定 $x_alpha in cal(F)(U_alpha)$, 满足 $U_alpha sect U_beta$ 上 $x_alpha$ 与 $x_beta$ 的限制相等, 就有唯一的 $x in cal(F)(U)$, 使得它限制在 $U_alpha$ 上是 $x_alpha$. 例如在连续函数层中, 这条公理意味着在一些开集上分别定义连续函数, 如果在定义域相交的地方这些函数相等, 那么就可以拼接成并集上的连续函数. 换句话说, 连续性是局部性质.

== Cartan–Eilenberg 革命

Cartan 与 Eilenberg 在 1950 年代著书 《同调代数》@CartanEilenberg, 为这个领域带来了一场革命.

描述交换群, 可以靠生成元和它们之间的关系. 具体来说, 如果有一组生成元构成的集合 $G$, 那么它们之间的关系就是 $ZZ^(plus.circle G)$ 的子群 $R$. 这也一定是自由交换群. 我们描述的群就是 $A = ZZ^(plus.circle G) \/ R$. 从这个描述出发, 可以方便地计算各种操作. 例如, 如果有两个交换群 $A, B$, 需要计算哪些交换群 $X$ 有子群 $B$, 满足商群 $X \/ B = A$. 那么可以考虑为 $A$ 的每个生成元 $g in G$ 取 $X$ 里的一个代表元. 那么, 对于生成元之间的关系 $r in R$, $X$ 中对应的元素在 $A$ 中的像就是零, 因此它唯一确定了 $B$ 中的一个元素. 这样我们就得到了一个同态 $phi : R -> B$.

这个函数也可以反过来确定 $X$. 考虑 $ZZ^(plus.circle G) times B$, 我们商去它的一个子群 $hat(R) = {(r, -phi(r)) | r in R}$, 就得到所需的群 $X$. 两个 $phi$ 确定同一个群扩张, 当且仅当它们的差可以表示为某个同态 $ZZ^(plus.circle G) -> B$ 限制到 $R$ 上. 因此, 我们可以将群扩张的集合定义为 $ "Ext"^1(A, B) = (ZZ^(plus.circle G) times B) \/ hat(R). $
这也揭示了群扩张的集合本身构成一个交换群. 这一点由 Baer 最早发现, 因此这个运算被称作 *Baer 和*.

这之中, 我们用到了交换群的*自由预解* (free resolution), 即一正合列 $dots.c -> F_2 -> F_1 -> A -> 0$, 其中 $F_i$ 都是自由交换群. 这可以看作是取定基底方便作具体计算的过程. 在交换群 (即 $ZZ$-模) 的情况, 自由预解总是在第二项就能停止, 但一般来说它可以任意长. 在这个计算中, 其实只用到了自由交换群的一条关键性质. 给定任何满射同态 $X ->> Y$, 从自由交换群出发的同态 $F -> Y$ 总能抬升为 $F -> X$, 使得得到的三角形交换. Cartan 与 Eilenberg 在他们的书中引入了这条性质, 称作*投射性*. 我们可以将自由预解替换为投射预解, 而不影响论证.

投射性有一条对偶性质, 即将定义中的所有箭头反向. 如果对于某个 $I$, 给定任何单射同态 $X >-> Y$, 任何同态 $X -> I$ 总能扩展为 $Y -> I$ 使得得到的三角形交换, 就称 $I$ *内射*. 这条性质也由 Baer 研究过. 对于交换群来说, 内射等价于*可除*, 即对于任何元素 $x$ 与正整数 $n$, 方程 $n y = x$ 都有解. 对偶地, 有内射预解 $0 -> A -> I_1 -> I_2 -> dots.c$ 的概念.

$"Ext", "Tor"$ 函子都可以用投射预解和内射预解定义. 这种操作的推广是*导出函子*. 这统一了同调代数中的许多概念.
